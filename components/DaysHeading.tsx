...
export default function DaysHeading() {
    ...
    return (
        <div key={`DaysHeadingMainContainer`} className="mt-10 grid grid-cols-7 gap-x-8 text-center gap-y-6 font-semibold ">
            ...
            {
                days.map((day, index)=>(
                    <Days day={day} key={`days-${day}`} index={index} isSunday={isSunday}/>
                ))
            }
        </div>
    )
